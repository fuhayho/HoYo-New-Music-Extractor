...
3. 如果没有 `.hdiff` 文件，但拥有想要进行比对和筛选的新版游戏 pck 文件，请将所有新文件放入 `./New Game Files` 文件夹，并运行脚本：`python main.py --no-hdiff`。也可以直接指定路径：
   - `python main.py --no-hdiff --original "path/to/original" --new "path/to/new"`
   - 如果任一文件夹为空，脚本会提示您输入路径（支持外部路径）。
4. 若需从 PCK 文件文件夹中提取**所有**音频而不进行比对，请运行 `python main.py --full "path/to/pck/folder"`。也可以单独运行 `python main.py --full`，随后根据提示输入路径。
...
输出编码参数为 `neroAacEnc -q 0.69`，因为 WAV 格式浪费磁盘空间，而 MP3 和 OGG 格式音质较差。请注意，输出完成后，`./Temp` 文件夹下的所有临时文件将被删除。 ---
